...
- (void)viewDidLoad
{
    ...
    simpleTable.separatorColor = [UIColor colorWithRed:1.0 green:182/255.0 blue:193/255.0 alpha:0.3];
    ...
    NSArray *adds = [self.TCnote queryWithData];
    NSEnumerator *subviewsEnum = [adds reverseObjectEnumerator];
    self.noteDatas = [NSMutableArray array];
    for (id obj in subviewsEnum) {
        //if ([obj isKindOfClass:self]) {
            [self.noteDatas addObject:obj];
       // }
    }
    //判断是删除模式（0），还是插入和移动模式（1）
    self.editMode = 0;
    ...
    // 隐藏垂直滚动条
        self.simpleTable.showsVerticalScrollIndicator = NO;
...
{
    //self.TCnote =  self.noteDatas[self.noteDatas.count- section -1]  ;
...
//        [tableView reloadRowsAtIndexPaths:[NSArray arrayWithObject:indexPath] withRowAnimation:UITableViewRowAnimationNone];
    ...
    if (editingStyle == UITableViewCellEditingStyleInsert)
    {
        ...
        if (self.TCnote.count > 26) {
            UIAlertView * alter = [[UIAlertView alloc] initWithTitle:@"提示" message:@"最多26条日程项~" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [alter show];
            return;
        }
        
        //更改列表文字
        [[self.TCnote.datas objectAtIndex:0] insertObject:@"新日程" atIndex:[indexPath row] + 1 ];
...
//- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    colorView.backgroundColor = [UIColor colorWithRed:1.0 green:182/255.0 blue:193/255.0 alpha:0.3];
    return colorView;
}
